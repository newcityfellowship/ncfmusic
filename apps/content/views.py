...
def export_registrations(request):
    ...
    for registration in ConferenceRegistration.objects.filter(has_paid=True).order_by('-insert_date'):
        row = [registration.insert_date.encode('utf-8').strip(), registration.first_name.encode('utf-8').strip(), registration.last_name.encode('utf-8').strip(), registration.email.encode('utf-8').strip(), registration.address.encode('utf-8').strip(), registration.city.encode('utf-8').strip(), registration.state.encode('utf-8').strip(), registration.postal_code.encode('utf-8').strip(), registration.country.encode('utf-8').strip(), registration.phone_number.encode('utf-8').strip(), registration.gender.encode('utf-8').strip(), registration.church_name.encode('utf-8').strip(), registration.how_serving.encode('utf-8').strip(), registration.skills.encode('utf-8').strip(), registration.wanting_to_learn.encode('utf-8').strip(), registration.housing.encode('utf-8').strip(), registration.special_housing_needs.encode('utf-8').strip(), registration.food_allergies.encode('utf-8').strip(), registration.ride_from_airport.encode('utf-8').strip(), registration.flight_information.encode('utf-8').strip(), registration.student.encode('utf-8').strip(), registration.cost.encode('utf-8').strip(),]
        writer.writerow(row)
        for registrant in ConferenceRegistrant.objects.filter(registration=registration):
            row = ['', registrant.first_name.encode('utf-8').strip(), registrant.last_name.encode('utf-8').strip(), registrant.email.encode('utf-8').strip(), '', '', '', '', '', '', registrant.gender.encode('utf-8').strip(), '', '', '' '', '', '', '', '', '', registrant.student.encode('utf-8').strip(),]
            writer.writerow(row)
