...
def export_registrations(request):
    ...
    for registration in ConferenceRegistration.objects.filter(has_paid=True).order_by('-insert_date'):
        row = [registration.insert_date, registration.first_name.encode('utf-8').strip(), registration.last_name.encode('utf-8').strip(), registration.email.encode('utf-8').strip(), registration.address.encode('utf-8').strip(), registration.city.encode('utf-8').strip(), registration.state.encode('utf-8').strip(), registration.postal_code, registration.country.encode('utf-8').strip(), registration.phone_number, registration.gender, registration.church_name.encode('utf-8').strip(), registration.how_serving.encode('utf-8').strip(), registration.skills.encode('utf-8').strip(), registration.wanting_to_learn.encode('utf-8').strip(), registration.housing, registration.special_housing_needs.encode('utf-8').strip(), registration.food_allergies.encode('utf-8').strip(), registration.ride_from_airport, registration.flight_information.encode('utf-8').strip(), registration.student, registration.cost,]
        writer.writerow(row)
